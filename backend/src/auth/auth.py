...
def check_permissions(permission, payload):
    #check if payload body contains permissions section (in its keys)
    if 'permissions' not in payload:
        raise AuthError({
            'code': 'invalid_token',
            'description': 'Permissions not included in JWT.'
        }, 400)
    #Now: check if that specific permission is not in the section permissions of the payload. because if not, means use is not authorized.
